...
* There are four main "sources" of interrupts:
    ...
    * Locality-specific (Locality-specific Peripheral Interrupt, or LPI): These are _always_ message-based interrupts which can be generated from an _Interrupt Translation Services_, or ITS. The Surface Pro machine in which this analysis was conducted on _does_ implement an ITS. However, because the OS is virtualized Hyper-V does not expose it to the root partition (thank you to [Longhorn](https://x.com/never_released) for pointing this out) However, this does not mean that message-based interrupts cannot still be generated and handled.
...
The Surface Pro machine in which this analysis was performed does not implement an ITS, but it still _does_ support LPIs (LPIs are supported but ITS is not required). GIC4 requires an ITS because GIC4 needs to support virtual LPIs due to support for direct injection of virtual interrupts to a VM without involving the hypervisor.
